...
### Normal Flow
1. **JWT Validation**: Original JWT validation runs as before
2. **Backend Call**: If validation succeeds and `jwt_service_url` is configured:
   - Makes GET request to your backend service
   - Passes through all original headers
   - Caches the response JWT
3. **Header Setting**: Sets `x-harmonic-cerberus-jwt` header with the fetched JWT
4. **Request Forwarding**: Original request continues to your upstream service
...
### Caching
- **Cache key**: Based on plugin configuration (shared across all requests)
- **Cache TTL**: Uses `expires_in` from backend response (default: 300 seconds)
- **Cache benefits**: Reduces load on backend service for subsequent requests
...
app.get('/get-jwt', (req, res) => {
  ...
  // Generate/fetch JWT based on the original headers
...
```bash
# Test your backend service directly with multiple headers
curl -X GET https://your-backend.com/get-jwt \
  -H "Authorization: Bearer your-original-jwt" \
  -H "X-Custom-Client-ID: client-123" \
  -H "User-Agent: MyApp/1.0"

# Your backend will receive ALL these headers and can use them to generate the JWT
...
- **Original JWT validation**: Still required - backend service is only called after successful validation
